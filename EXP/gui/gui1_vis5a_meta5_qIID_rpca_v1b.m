function varargout = gui1_vis5a_meta5_qIID_rpca_v1b(varargin)
% GUI1_VIS5A_META5_QIID_RPCA_V1B MATLAB code for gui1_vis5a_meta5_qIID_rpca_v1b.fig
%      GUI1_VIS5A_META5_QIID_RPCA_V1B, by itself, creates a new GUI1_VIS5A_META5_QIID_RPCA_V1B or raises the existing
%      singleton*.
%
%      H = GUI1_VIS5A_META5_QIID_RPCA_V1B returns the handle to a new GUI1_VIS5A_META5_QIID_RPCA_V1B or the handle to
%      the existing singleton*.
%
%      GUI1_VIS5A_META5_QIID_RPCA_V1B('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUI1_VIS5A_META5_QIID_RPCA_V1B.M with the given input arguments.
%
%      GUI1_VIS5A_META5_QIID_RPCA_V1B('Property','Value',...) creates a new GUI1_VIS5A_META5_QIID_RPCA_V1B or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before gui1_vis5a_meta5_qIID_rpca_v1b_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to gui1_vis5a_meta5_qIID_rpca_v1b_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help gui1_vis5a_meta5_qIID_rpca_v1b

% Last Modified by GUIDE v2.5 03-Oct-2019 18:52:59

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @gui1_vis5a_meta5_qIID_rpca_v1b_OpeningFcn, ...
    'gui_OutputFcn',  @gui1_vis5a_meta5_qIID_rpca_v1b_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before gui1_vis5a_meta5_qIID_rpca_v1b is made visible.
function gui1_vis5a_meta5_qIID_rpca_v1b_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to gui1_vis5a_meta5_qIID_rpca_v1b (see VARARGIN)

% Choose default command line output for gui1_vis5a_meta5_qIID_rpca_v1b
...
handles.E1 = im2double(imread(fullfile(handles.Rpath,[handles.Iname '_iE1.png'])));
handles.E2 = im2double(imread(fullfile(handles.Rpath,[handles.Iname '_iE2.png'])));
handles.E3 = im2double(imread(fullfile(handles.Rpath,[handles.Iname '_iE3.png'])));
handles.E4 = im2double(imread(fullfile(handles.Rpath,[handles.Iname '_iE4.png'])));
handles.E5 = im2double(imread(fullfile(handles.Rpath,[handles.Iname '_iE5.png'])));
...
% handles.imRecon = zeros(size(handles.I_c));
...
% UIWAIT makes gui1_vis5a_meta5_qIID_rpca_v1b wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = gui1_vis5a_meta5_qIID_rpca_v1b_OutputFcn(hObject, eventdata, handles)
...
% A{1} = handles.A1; A{2} = handles.A2; A{3} = handles.A3; A{4} = handles.A4; A{5} = handles.A5;
% E{1} = handles.E3; E{2} = handles.E4;
% [~,pMaps_1] = getFocusPmaps(E,5,0.1,1,0.01);
% allL = zeros(size(handles.I_s));
% allL = allL +  A{3} .* repmat([pMaps_1(:,:,1)],[1 1 3]);
% allL = allL +  A{4} .* repmat([pMaps_1(:,:,2)],[1 1 3]);
% 
% imRecon = (handles.alpha*allL + handles.beta*(handles.gamma1*(handles.E1+handles.E2) + handles.gamma2*(handles.E5))/(handles.gamma1 + handles.gamma2)).*handles.I_a;

A{1} = handles.A1; A{2} = handles.A2; A{3} = handles.A3; A{4} = handles.A4; A{5} = handles.A5;
E{1} = handles.E1; E{2} = handles.E2; E{3} = handles.E3; E{4} = handles.E4; E{5} = handles.E5;
[~,pMaps_1] = getFocusPmaps(E,5,0.1,1,0.01);
allL = zeros(size(handles.I_s));
allL = allL +  A{1} .* repmat([pMaps_1(:,:,1)],[1 1 3]);
allL = allL +  A{2} .* repmat([pMaps_1(:,:,2)],[1 1 3]);
allL = allL +  A{3} .* repmat([pMaps_1(:,:,3)],[1 1 3]);
allL = allL +  A{4} .* repmat([pMaps_1(:,:,4)],[1 1 3]);
allL = allL +  A{5} .* repmat([pMaps_1(:,:,5)],[1 1 3]);
imRecon = allL.*handles.I_a;
imRecon = (handles.I_c + imRecon)/2;
imRecon = handles.alpha*imRecon + handles.beta*(handles.gamma1*E{1} + handles.gamma2*E{2} + handles.gamma3*E{3} + handles.gamma4*E{4} + handles.gamma5*E{5});
% imRecon = (handles.alpha*allL + handles.beta*(handles.gamma1*(handles.E1+handles.E2) + handles.gamma2*(handles.E5))/(handles.gamma1 + handles.gamma2)).*handles.I_a;
...
handles.A5 = normalizeIm(handles.A5);
% handles.E1 = normalizeIm(handles.E1);
% handles.E2 = normalizeIm(handles.E2 - handles.E1);
% handles.E3 = normalizeIm(handles.E3 - handles.E2);
% handles.E4 = normalizeIm(handles.E4 - handles.E3);
% handles.E5 = normalizeIm(handles.E5 - handles.E4);
handles.E1 = normalizeIm(handles.E1);
handles.E2 = normalizeIm(handles.E2);
handles.E3 = normalizeIm(handles.E3);
handles.E4 = normalizeIm(handles.E4);
