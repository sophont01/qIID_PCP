function[A1,E1] = func_qIID_rpca_v2b(I,k,p)
% function[iA1, iE1, iA2, iE2, iA3, iE3, iA4, iE4, iA5, iE5] = func_qIID_rpca_v2b(I,k,p)
pp = floor(p/2);
pI = padarray(I,[pp,pp],'replicate');
Ipatches = im2patches(pI,[p,p],1);
x = reshape(Ipatches(:,:,1,:),p*p,[]);
y = reshape(Ipatches(:,:,2,:),p*p,[]);
z = reshape(Ipatches(:,:,3,:),p*p,[]);
w = zeros(size(x));
qI = quaternion(w,x,y,z);
[A1,E1] = inexact_alm_qrpca(qI, k/sqrt(length(qI)), 1e-7, 1000);
